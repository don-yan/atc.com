...
(function(global) {
    function init(global) {
        ...
        if (!widgets.foundElements)
            widgets.foundElements = [];
        ...
        for (var i = 0; i < elements.length; i++) {
            ...
            // NOTE: YK-EDIT ... get rid of parent element restriction so that we can load this async
            //            if (element.src.match(re) && found.indexOf(element) < 0 && (element.parentNode.className === 'tt-widget' || element.getAttribute('type') === 'inline')) {
            if (element.src.match(re) && found.indexOf(element) < 0) {
                found.push(element);
                return element;
            }
        }
    ...
    function startWidget(global, target) {
        var iFrameUrl = target.getAttribute('data-url');
        ...
        if (refValue !== null && iFrameUrl.indexOf('ref') === -1) {
            iFrameUrl += "&ref=" + refValue;
        }
        if (target.getAttribute('data-inline-minimal') == 'true') {
            iFrameUrl += "&minimal=true";
        } else {
            iFrameUrl += "&minimal=false";
        }
        if (target.getAttribute('data-inline-show-logo') == 'true' || target.getAttribute('data-inline-show-logo') == '1') {
            iFrameUrl += "&show_logo=true";
        } else {
            iFrameUrl += "&show_logo=false";
        }
        if (target.getAttribute('data-inline-bg-fill') == 'true') {
            iFrameUrl += "&bg_fill=true";
        } else {
            iFrameUrl += "&bg_fill=false";
        }
        if (target.getAttribute('data-show_search_filter') == 'true') {
            iFrameUrl += "&show_search_filter=true";
        }
        if (target.getAttribute('data-show_date_filter') == 'true') {
            iFrameUrl += "&show_date_filter=true";
        }
        if (target.getAttribute('data-show_sort') == 'true') {
            iFrameUrl += "&show_sort=true";
        }
        if (target.getAttribute('inline-search') !== null) {
            iFrameUrl += "&srch=" + target.getAttribute('data-inline-search');
        }
        if (getLinkerParam()) {
            iFrameUrl += "&" + getLinkerParam();
        }
            ...
            function getClientId(uaNumber) {
                ...
                if (trackers !== null) {
                    ...
                    for (i = 0; i < trackers.length; i++) {
                        var _tracker = trackers[i];
                        if (!uaNumber || _tracker.get('trackingId') === uaNumber) {
                            let trackerParam = _tracker.get('clientId');
                            console.log({
                                trackerParam
                            })
                            return trackerParam;
